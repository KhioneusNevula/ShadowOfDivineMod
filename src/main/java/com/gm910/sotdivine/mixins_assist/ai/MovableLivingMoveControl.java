...
public class MovableLivingMoveControl extends DummyMoveControl {

	private Mob controller;

	public MovableLivingMoveControl(LivingEntity mob, Mob controller) {
		super(mob);
	...
	public void tick() {
		if (this.operation == MoveControl.Operation.STRAFE) {
			...
			if (d2 > this.mob.maxUpStep() && d0 * d0 + d1 * d1 < Math.max(1.0F, this.mob.getBbWidth())
				...
				this.controller.getJumpControl().jump();
	...
	private boolean isWalkable(float p_24997_, float p_24998_) {
		PathNavigation pathnavigation = this.controller.getNavigation();
		if (pathnavigation != null) {
			NodeEvaluator nodeevaluator = pathnavigation.getNodeEvaluator();
			if (nodeevaluator != null
					&& nodeevaluator.getPathType(this.controller, BlockPos.containing(this.mob.getX() + p_24997_,
							this.mob.getBlockY(), this.mob.getZ() + p_24998_)) != PathType.WALKABLE) {
				return false;
			}
		}
