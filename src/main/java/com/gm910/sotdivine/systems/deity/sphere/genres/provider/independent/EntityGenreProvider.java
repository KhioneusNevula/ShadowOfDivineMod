...
import com.gm910.sotdivine.systems.deity.sphere.genres.creator.EntityCreator;
...
import com.gm910.sotdivine.systems.deity.sphere.genres.provider.data.other.NbtInternalProvider;
import com.gm910.sotdivine.systems.deity.sphere.genres.provider.entity.EntityFlagsGenreProvider;
import com.gm910.sotdivine.systems.deity.sphere.genres.provider.entity.EquipmentGenreProvider;
import com.gm910.sotdivine.systems.deity.sphere.genres.provider.entity.MobEffectGenreProvider;
import com.gm910.sotdivine.systems.deity.sphere.genres.provider.entity_preds.ITypeSpecificProvider;
...
public record EntityGenreProvider(EntityTypePredicate entityType, Collection<MobEffectGenreProvider> effects,
		...
		ComponentMapProvider components, Map<ResourceLocation, ITypeSpecificProvider<?>> typeSpecific)
	...
	public EntityGenreProvider(EntityTypePredicate type) {
		this(type, Set.of(), Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty(),
				ComponentMapProvider.ANY, Map.of());
	}
	...
	public static Codec<EntityGenreProvider> codec() {
		if (CODEC == null) {
			...
			Codec<EntityGenreProvider> construction = Codec.recursive("EntityGenreProvider",
					(recurse) -> RecordCodecBuilder.create(instance -> instance
							.group(EntityTypePredicate.CODEC.fieldOf("entities")
									.forGetter(EntityGenreProvider::entityType),
									Codec.list(MobEffectGenreProvider.codec()).optionalFieldOf("effects", List.of())
											.forGetter((x) -> new ArrayList<>(x.effects)),
									NbtInternalProvider.CODEC.optionalFieldOf("nbt")
											.forGetter(EntityGenreProvider::nbt),
									EntityFlagsGenreProvider.CODEC.optionalFieldOf("flags")
											.forGetter(EntityGenreProvider::flags),
									EquipmentGenreProvider
											.codec().optionalFieldOf("equipment")
									...
									Codec.<ResourceLocation, ITypeSpecificProvider<?>>dispatchedMap(
											ResourceLocation.CODEC, TypeSpecificProviders::getCodec)
											.optionalFieldOf("type_specific", Map.of())
											.forGetter((x) -> x.typeSpecific))
	...
	public boolean matches(ServerLevel level, Entity instance) {
		...
		if (!typeSpecific.isEmpty()) {
			if (!typeSpecific.values().stream().allMatch((x) -> x.entityClass().isAssignableFrom(instance.getClass())
	...
	public EntityCreator generateRandom(ServerLevel level, Optional<Entity> prev) {
		...
		Consumer<? extends Entity> consumer = (e) -> {
			...
			if (!typeSpecific.isEmpty()) {
				typeSpecific.values().forEach((ts) -> {
	...
	public final String toString() {
		return "EntityGenreProvider(" + entityType.types().unwrap().map(Object::toString, Object::toString) + ")";
	}

	@Override
	public String report() {
		return "EntityGenreProvider{types=" + entityType().types().unwrap().map(Object::toString, Object::toString)
